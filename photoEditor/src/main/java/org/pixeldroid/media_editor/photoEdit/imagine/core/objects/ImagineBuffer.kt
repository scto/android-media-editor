...
import org.pixeldroid.media_editor.photoEdit.imagine.core.objects.ImagineBuffer.Index
import org.pixeldroid.media_editor.photoEdit.imagine.core.objects.ImagineBuffer.Vertex
import org.pixeldroid.media_editor.photoEdit.imagine.util.getProxyInt
import org.pixeldroid.media_editor.photoEdit.imagine.util.setProxyInt

/**
 * A wrapper for an OpenGL Buffer Object with allocation safety. Being a sealed class, it has 2
 * variants, [Vertex] and [Index] to store [GLES20.GL_ARRAY_BUFFER] and
 * [GLES20.GL_ELEMENT_ARRAY_BUFFER] respectively
 *
 * @property handle Reference to the underlying Buffer Object
 */
internal sealed class ImagineBuffer @VisibleForTesting constructor(protected val handle: Int) {
    ...
    fun release() = releaseSafe {
        setProxyInt(handle) { GLES20.glDeleteBuffers(1, it, 0) }

        isReleased = true
    }

    /** Bind this Buffer Object during drawing. Implemented in the variants */
    abstract fun bind()
    ...
    class Vertex(handle: Int) : ImagineBuffer(handle) {
        ...
        override fun bind() = releaseSafe { GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, handle) }
    }
    ...
    companion object {
        ...
        /**
         * Safely allocate an OpenGL [GLES20.GL_ARRAY_BUFFER] from a given set of vertices and wrap
         * it in [Vertex] variant
         *
        ...
        fun createVertex(vertices: FloatArray): Vertex {
            val handle = getProxyInt { GLES20.glGenBuffers(1, it, 0) }

            // Create a directly allocated Buffer in memory
            // and put the vertex data in it
            val buffer =
                ByteBuffer.allocateDirect(Float.SIZE_BYTES * vertices.size)
        ...
        /**
         * Safely allocate an OpenGL [GLES20.GL_ELEMENT_ARRAY_BUFFER] from a given set of indices
         * and wrap it in [Index] variant
         *
        ...
        fun createIndex(indices: ShortArray): Index {
            ...
            val buffer =
                ByteBuffer.allocateDirect(Short.SIZE_BYTES * indices.size)
                    .order(ByteOrder.nativeOrder())
                    .asShortBuffer()
                    .put(indices)
                    .position(0)
