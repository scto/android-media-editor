...
import org.pixeldroid.media_editor.photoEdit.imagine.core.objects.ImagineLayerShaderFactory.Companion.create
import org.pixeldroid.media_editor.photoEdit.imagine.core.types.ImagineLayer
import org.pixeldroid.media_editor.photoEdit.imagine.util.readRawRes
 ...
 * @property vsQuad The partial pre-compiled Vertex Shader to link with new layer Fragment Shaders
 *   at runtime
 */
internal class ImagineLayerShaderFactory
@VisibleForTesting
constructor(
    ...
    /**
     * Maps [ImagineLayer] descendant class names to their corresponding [ImagineLayerShader] for
     * ease of access. Acts as a cache
     */
    private val layerShaderMap: MutableMap<Int, ImagineLayerShader> = mutableMapOf()

    /**
     * Tries to supply an [ImagineLayerShader] for a particular [ImagineLayer]. It either tries to
     * fetch from the cache, or else tries creating it
     *
    ...
    fun getLayerShader(layer: ImagineLayer): ImagineLayerShader? = releaseSafe {
        layerShaderMap[layer.shaderId()]
            ?: run {
                // Optimization to prevent resize
                val fsLayerSrc =
                    StringBuilder(fsLayerBaseSrc.length + layer.source.length + 2)
                        .apply {
                ...
                val fsLayer =
                    ImagineShader.compile(fsLayerSrc, ImagineShader.Type.Fragment)
                        ?: return@run null

                val program = fsLayer.linkWith(vsQuad, true) ?: return@run null

                ImagineLayerShader(program).also { layerShaderMap[layer.shaderId()] = it }
            }
    }
    ...
    companion object {
        ...
        fun create(context: Context): ImagineLayerShaderFactory? {
            ...
            val vsQuad = ImagineShader.compile(vsQuadSrc, ImagineShader.Type.Vertex) ?: return null
            val fsLayerCopy =
                ImagineShader.compile(fsLayerCopySrc, ImagineShader.Type.Fragment) ?: return null

            val layerCopyShader =
                fsLayerCopy.linkWith(vsQuad, releaseOnFailure = true)
                    ?: run {
                        vsQuad.release()
                        return null
                    }
