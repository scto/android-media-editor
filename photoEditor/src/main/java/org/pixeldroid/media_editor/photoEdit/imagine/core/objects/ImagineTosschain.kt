...
import org.pixeldroid.media_editor.photoEdit.imagine.core.objects.ImagineTosschain.Companion.create
import org.pixeldroid.media_editor.photoEdit.imagine.core.types.ImagineDimensions

/**
 * A construct used to maintain two framebuffers, each having identical textures attached to their
 * color attachments.
 *
 * You cannot directly instantiate this class. Instead use the companion function [create] to do so.
 *
 ...
 */
internal class ImagineTosschain
@VisibleForTesting
constructor(
    ...
    /** Obtain a bitmap from the pixels represented in the currently bound framebuffer */
    val bitmap: Bitmap
        get() =
            ByteBuffer.allocateDirect(dimensions.width * dimensions.height * COLOR_COMPONENT_COUNT)
                .let { buffer ->
                    GLES20.glReadPixels(
                        0,
                        0,
